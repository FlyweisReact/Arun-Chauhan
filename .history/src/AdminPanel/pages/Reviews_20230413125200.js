  ...
  const fetchData = async () => {
    try {
      const { data } = await axios.get(
        "https://oyi65hi3pd.execute-api.ap-south-1.amazonaws.com/development/api/review"
      );
      setData(data.data);
  ...
  const putHandler = async (id) => {
    try {
      const { data } = await axios.post(
        `https://oyi65hi3pd.execute-api.ap-south-1.amazonaws.com/development/api/review/${id}`
      );
      console.log(data);
