  ...
  const fetchData = async () => {
    try {
      const { data } = await axios.get(
        `https://oyi65hi3pd.execute-api.ap-south-1.amazonaws.com/development/api/feedback/`
      );
      setData(data.result);
    } catch (e) {
      console.log(e);
  ...
  const deleteHandler = async (id) => {
    try {
      const { data } = await axios.delete(
        `https://oyi65hi3pd.execute-api.ap-south-1.amazonaws.com/development/api/feedback/${id}`
      );
      console.log(data);
