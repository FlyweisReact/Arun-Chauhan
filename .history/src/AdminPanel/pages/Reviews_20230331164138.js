...
const Reviews = () => {
  ...
  const putHandler = async (e) => {
    e.preventDefault()
    try{
      const { data } = await axios.put(`http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:1112/api/review/6419928ca4762f89be725401`)
    }catch(e){
      console.log(e)
    }
  }
            ...
            <tbody>
              {data?.map((i, index) => (
                <tr key={index}>
                  ...
                  <td>
                   <Button variant="outline-success">Change Status</Button>
                  </td>
                </tr>
              ))}
            </tbody>
