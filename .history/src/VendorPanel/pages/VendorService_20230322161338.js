...
const VendorService = () => {
  ...
  const fetchSub = async () => {
    try{
      const { data} = await axios.get("http://ec2-15-206-210-177.ap-south-1.compute.amazonaws.com:1112/api/subcategory/")
          ...
          <Form onSubmit={postHandler}>
            ...
            <Form.Group className="mb-3">
              ...
              >
                ...
                {subCat?.map((i , index) => (
                  <option key={index} value={i._id}> {i.title} </option>
                ))}
              </Form.Select>
            </Form.Group>
            <Form.Group className="mb-3">
